...
r <- rast(water_thresh_list[length(water_thresh_list)])
leaflet() |> addTiles() |>
  addRasterImage(r)
```
...
water_val_list <- lapply(water_rast_list, function(x) {
  ...
  dte <- gsub("water_", "", dte)
  dte <- gsub(".tif", "", dte)
  val_df <- data.frame("Image_date" = dte,
                       "Water_Index" = val)
  names(val_df) <- c("Image", "Water_value")
  return(val_df)
})
...
knitr::kable(vals_df)
ggplot(vals_df) +
  geom_col(aes(x=Image, y=Water_value), fill = "blue") +
  ggtitle("Water Index", subtitle = paste("at location", coords$lon, coords$lat))
```
